...
namespace mlp_d
{
  ...
  // Structs and Classes
    ...
    class SGD
    {
      ...
      std::vector<double> gradientCrossEntWthSftMax(const std::vector<double> &logits,
 ...
 what matters is that the last input was 'exit' + enter key or escape key.\n";
    ...
    std::size_t getLabel(NeuralNetwork &NN)
    {
      std::size_t layers_len = NN.layers_list.size();
      std::vector<double> last_layer_actvs = NN.NN_layers[layers_len-1]->actvs;
      std::size_t last_layer_len = NN.NN_layers[layers_len - 1]->getNum();
      double max = std::numeric_limits<double>::min();
      std::size_t max_index = -1;

      for (std::size_t i = 0; i < last_layer_len; ++i)
      {
        bool check = last_layer_actvs[i] > (max);
        max = check ? last_layer_actvs[i] : max;
    ...
    void SGD::initSGD(std::size_t batch_offset, double _eta, std::vector<double> &prev_g_d_arr)
    {
      ...
      if(running.load())
      {
        ...
        for (std::size_t i = 0; i < batch_size; ++i)
        {
          add_vectors_neon_double(gradients, thread_gradients_ptrs[i], gradients, 0, gradients.size());
        }
        ...
        div_vectors_neon_double(gradients, batch_size_vec, gradients, 0, gradients.size());

        prev_g_d_arr.assign(prev_g_d_arr.size(), 0.0);
        add_vectors_neon_double(prev_g_d_arr, gradients, prev_g_d_arr, 0, prev_g_d_arr.size());
