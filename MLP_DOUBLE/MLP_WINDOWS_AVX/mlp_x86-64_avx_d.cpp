...
namespace mlp_d
{
  ...
  // Main Functions Declaration
    ...
    void add_vectors_avx_double(const std::vector<double> &a, const std::vector<double> &b, std::vector<double> &res, std::size_t start, std::size_t end);
    void sub_vectors_avx_double(const std::vector<double> &a, const std::vector<double> &b, std::vector<double> &res, std::size_t start, std::size_t end);
    void mul_vectors_avx_double(const std::vector<double> &a, const std::vector<double> &b, std::vector<double> &res, std::size_t start, std::size_t end);
    void div_vectors_avx_double(const std::vector<double> &a, const std::vector<double> &b, std::vector<double> &res, std::size_t start, std::size_t end);
    ...
    template <typename Container> void printVector(Container &vec);
 ...
 what matters is that the last input was 'exit' + enter key or escape key.\n";
    ...
    void add_vectors_avx_double(const std::vector<double> &a, const std::vector<double> &b, std::vector<double> &res, std::size_t start, std::size_t end)
    {
      ...
      // Process groups of 4 doubles using AVX
      for(; i + 3 < n; i+=4)
      {
        __m256d va = _mm256_loadu_pd(&a[i]); // Load 4 doubles from vector a
        __m256d vb = _mm256_loadu_pd(&b[i]); // Load 4 doubles from vector b
        __m256d vr = _mm256_add_pd(va, vb); // Add the 4 doubles
        _mm256_store_pd(&res[i], vr); // Store the result back to result vector
      }
    ...
    void sub_vectors_avx_double(const std::vector<double> &a, const std::vector<double> &b, std::vector<double> &res, std::size_t start, std::size_t end)
    {
      ...
      for(; i + 3 < n; i+=4)
      {
        __m256d va = _mm256_loadu_pd(&a[i]); // Load 4 doubles from vector a
        __m256d vb = _mm256_loadu_pd(&b[i]); // Load 4 doubles from vector b
        __m256d vr = _mm256_sub_pd(va, vb); // Subtract the 4 doubles
        _mm256_store_pd(&res[i], vr); // Store the result back to result vector
      }
    ...
    void mul_vectors_avx_double(const std::vector<double> &a, const std::vector<double> &b, std::vector<double> &res, std::size_t start, std::size_t end)
    {
      ...
      for(; i + 3 < n; i+=4)
      {
        __m256d va = _mm256_loadu_pd(&a[i]); // Load 4 doubles from vector a
        __m256d vb = _mm256_loadu_pd(&b[i]); // Load 4 doubles from vector b
        __m256d vr = _mm256_mul_pd(va, vb); // Multiply the 4 doubles
        _mm256_store_pd(&res[i], vr); // Store the result back to result vector
      }
    ...
    void div_vectors_avx_double(const std::vector<double> &a, const std::vector<double> &b, std::vector<double> &res, std::size_t start, std::size_t end)
    {
      ...
      for(; i + 3 < n; i+=4)
      {
        __m256d va = _mm256_loadu_pd(&a[i]); // Load 4 doubles from vector a
        __m256d vb = _mm256_loadu_pd(&b[i]); // Load 4 doubles from vector b
        __m256d vr = _mm256_div_pd(va, vb); // Divide the 4 doubles
        _mm256_store_pd(&res[i], vr); // Store the result back to result vector
      }
    ...
    std::vector<double> sliceDblVec(const std::vector<double> &vec, std::size_t start, std::size_t end)
    {
      if (start > end || (end - start) > vec.size())
      {
        std::cout << "\nStart: " << start << ", End: " << end << ", Len: " << vec.size() << "\n" << std::endl;
        std::cerr << "Invalid slice indices." << std::endl;
        running.store(false);
    ...
    void NN2Vec(NeuralNetwork* NN, std::vector<double> &arr, std::size_t offset, int flag)
    {
      ...
      for (std::size_t i = 1; i < layers_len; ++i)
      {
        ...
        if (flag >= 0)
        {
          ...
          add_vectors_avx_double(sliced_arr, parameters, sliced_arr, 0, sliced_arr.size());
        }

        else
        {
          sub_vectors_avx_double(sliced_arr, parameters, sliced_arr, 0, sliced_arr.size());
        }
    ...
    void Vec2NN(std::vector<double> &arr, NeuralNetwork* NN, std::size_t offset, int flag)
    {
      ...
      for (std::size_t i = 1; i < layers_len; ++i)
      {
        ...
        if(flag >= 0)
        {
          ...
          add_vectors_avx_double(parameters, sliced_arr, parameters, 0, parameters.size());
        }

        else
        {
          sub_vectors_avx_double(parameters, sliced_arr, parameters, 0, parameters.size());
        }
    ...
    std::vector<std::uint8_t> readImage(const char *filename, std::size_t row, std::size_t pix_size)
    {
      ...
      {
        fseek(file, 16 + (row * pix_size), SEEK_SET);
        fread(x.data(), sizeof(char), pix_size, file);
        fclose(file);
    ...
    double h_dropout, std::vector<double> &preactvs, std::vector<double> &actvs) 
    {
      ...
      for (std::size_t i = 0; i < layers_len; ++i)
      {
        ...
        { 
          ...
          std::vector<double> 
          last_actvs = sliceDblVec(actvs, start - p_len, start),
          ...
          if (i == layers_len - 1)
          {
            ...
            add_vectors_avx_double(actvs, stored_actvs, actvs, 0, actvs.size()); 
          } 
...
// DLL Helper Functions Declaration
  ...
  extern "C" int __cdecl NN(const char *mlp_filename,  double _i_dropout, double _h_dropout, uint8_t *_image_data, unsigned _image_data_size, bool _verbose);
  extern "C" void __cdecl trainNN1(unsigned *layers_list, unsigned layers_len, unsigned _train_len, double _eta, unsigned _batch_size, unsigned _epoch_num,
  ...
  extern "C" void __cdecl trainNN2(const char *mlp_filename, unsigned _train_len, double _eta, unsigned _batch_size, unsigned _epoch_num,
  double _alpha, double _lambda,  double _i_dropout, double _h_dropout, const char *train_label_file, const char *train_image_file, bool _verbose);
  ...
  extern "C" void __cdecl testNN(const char *mlp_filename, unsigned _test_len,  double _i_dropout, double _h_dropout, const char *test_label_file,
  const char *test_image_file, bool _verbose, bool validate);
...
// DLL Helper Functions Implementation
  ...
  extern "C" int __cdecl NN(const char *mlp_filename, double i_dropout, double h_dropout, std::uint8_t *_image_data, unsigned _image_data_size, bool _verbose)
  {
    using namespace std::chrono;
    const auto start{steady_clock::now()};
  ...
  extern "C" void __cdecl testNN(const char *mlp_filename, unsigned _test_len, double i_dropout, double h_dropout, const char *test_label_file,
  const char *test_image_file, bool _verbose, bool validate)
  {
    using namespace std::chrono;
    const auto start{steady_clock::now()};
